...
function HasPerms(member, query) {
    if (member.id === config_1.methode_id || member.id === config_1.kira_id) {
        return true;
    }
    var command = FindCommand(query);
    var hasPerm = false;
    ...
    if (command && command.perms) {
        command.perms.map(function (permName) {
            var perm = config_1.perms.find(function (p) { return p.name === permName.toLowerCase(); });
            if (perm) {
                commandPerms.push(perm);
            }
        });
    }
    if (!command) {
    ...
    for (var i = 0; i < commandPerms.length; i++) {
        var perm = commandPerms[i];
